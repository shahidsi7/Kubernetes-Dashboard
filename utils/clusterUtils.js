...
// This file contains utility functions related to cluster setup and management,
// including the multi-step process for installing the AWS Load Balancer Controller.

const fs = require('fs/promises');
const path = require('path');
const os = require('os');
const { execPromise, getAwsAccountId } = require("./kubectlUtils");
const https = require('https');

/**
 * Applies a default EBS StorageClass to the cluster.
 * This ensures that dynamic volume provisioning works out-of-the-box for PVCs.
 * @param {string} clusterName - The name of the cluster.
 * @param {string} region - The region of the cluster.
 * @param {WebSocket} ws - The WebSocket to send logs to.
 */
async function applyDefaultStorageClass(clusterName, region, ws) {
...
`;
    try {
        ws.send(JSON.stringify({ type: 'log', data: '\n--- Applying default EBS StorageClass (ebs-sc) ---\n' }));
        await execPromise(`eksctl utils write-kubeconfig --cluster ${clusterName} --region ${region}`);
        await execPromise(`kubectl apply -f -`, { input: storageClassManifest });
        ws.send(JSON.stringify({ type: 'log', data: 'Default EBS StorageClass applied successfully.\n' }));
    } catch (error) {
        const errorMessage = `Failed to apply default EBS StorageClass: ${error.message}`;
        console.error(errorMessage);
        ws.send(JSON.stringify({ type: 'log', data: `\x1b[31m${errorMessage}\x1b[0m\n` }));
    }
}

/**
 * Helper function to download content from a URL using Node's native https module.
 * @param {string} url - The URL to download from.
 * @returns {Promise<string>} A promise that resolves with the content of the URL.
 */
function downloadManifest(url) {
    return new Promise((resolve, reject) => {
        https.get(url, (res) => {
            if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
                return downloadManifest(res.headers.location).then(resolve).catch(reject);
            }
            if (res.statusCode < 200 || res.statusCode >= 300) {
                return reject(new Error(`Failed to download manifest, status code: ${res.statusCode}`));
            }
            let data = '';
...
/**
 * NEW: Installs cert-manager, a prerequisite for the AWS Load Balancer Controller.
 * @param {WebSocket} ws - The WebSocket to send logs to.
 * @returns {Promise<boolean>} A promise that resolves to true on success.
 */
async function installCertManager(ws) {
    ws.send(JSON.stringify({ type: 'log', data: `\n--- Installing cert-manager for ALB Controller ---\n` }));
    try {
        const certManagerUrl = 'https://github.com/cert-manager/cert-manager/releases/download/v1.13.2/cert-manager.yaml';
        ws.send(JSON.stringify({ type: 'log', data: `Applying cert-manager manifest from ${certManagerUrl}...\n` }));
        
        // Using kubectl apply directly with the URL is the most reliable method
        await execPromise(`kubectl apply -f ${certManagerUrl}`);
        
        ws.send(JSON.stringify({ type: 'log', data: 'cert-manager manifest applied successfully.\n' }));
        
        // Wait for the cert-manager webhook to become ready before proceeding. This is crucial.
        ws.send(JSON.stringify({ type: 'log', data: 'Waiting for cert-manager webhook to be ready...\n' }));
        await execPromise('kubectl wait --for=condition=ready pod -l app.kubernetes.io/instance=cert-manager -n cert-manager --timeout=5m');
        
        ws.send(JSON.stringify({ type: 'log', data: 'cert-manager is ready.\n' }));
        return true;
    } catch (error) {
        const errorMessage = `Failed to install or verify cert-manager: ${error.message}`;
        console.error(errorMessage);
        ws.send(JSON.stringify({ type: 'log', data: `\x1b[31m${errorMessage}\x1b[0m\n` }));
        return false;
    }
...
/**
 * Creates the required IAM Policy for the AWS Load Balancer controller.
 * It first checks if the policy already exists to avoid errors.
 * @param {WebSocket} ws - The WebSocket to send logs to.
 * @returns {Promise<string>} A promise that resolves with the ARN of the policy.
 */
async function createOrVerifyAlbIamPolicy(ws) {
    ws.send(JSON.stringify({ type: 'log', data: `\n--- Preparing IAM Policy for AWS Load Balancer Controller ---\n` }));
    try {
        const accountId = await getAwsAccountId();
        const policyName = 'AWSLoadBalancerControllerIAMPolicy';
        const policyArn = `arn:aws:iam::${accountId}:policy/${policyName}`;
        
        ws.send(JSON.stringify({ type: 'log', data: `Checking if policy ${policyName} exists...\n` }));
        
        try {
            await execPromise(`aws iam get-policy --policy-arn ${policyArn}`);
            ws.send(JSON.stringify({ type: 'log', data: `IAM policy '${policyName}' already exists. Skipping creation.\n` }));
            return policyArn;
        } catch (error) {
            ws.send(JSON.stringify({ type: 'log', data: `IAM policy not found. Creating policy '${policyName}'...\n` }));
            ...
            ws.send(JSON.stringify({ type: 'log', data: `Downloading IAM policy from ${policyUrl}...\n` }));
            const iamPolicyJson = await downloadManifest(policyUrl);

            const tempFilePath = path.join(os.tmpdir(), 'iam-policy.json');
            await fs.writeFile(tempFilePath, iamPolicyJson);
            
            await execPromise(`aws iam create-policy --policy-name ${policyName} --policy-document file://${tempFilePath}`);
            
            await fs.unlink(tempFilePath);
            
            ws.send(JSON.stringify({ type: 'log', data: `Successfully created IAM policy '${policyName}'.\n` }));
            return policyArn;
        }
    } catch (error) {
        const errorMessage = `Failed to create or verify IAM policy for ALB Controller: ${error.message}`;
        console.error(errorMessage);
        ws.send(JSON.stringify({ type: 'log', data: `\x1b[31m${errorMessage}\x1b[0m\n` }));
        throw error;
    }
...
/**
 * Creates the IAM Service Account, now using a specific policy ARN.
 * @param {string} clusterName - The name of the cluster.
 * @param {string} region - The region of the cluster.
 * @param {string} policyArn - The ARN of the IAM policy to attach.
 * @param {WebSocket} ws - The WebSocket to send logs to.
 * @returns {Promise<boolean>} A promise that resolves to true on success.
 */
async function createAlbIamServiceAccount(clusterName, region, policyArn, ws) {
    const command = `eksctl create iamserviceaccount --name aws-load-balancer-controller --namespace kube-system --cluster ${clusterName} --region ${region} --attach-policy-arn ${policyArn} --approve --override-existing-serviceaccounts`;
    const maxRetries = 3;
    const retryDelay = 15000;

    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ws.send(JSON.stringify({ type: 'log', data: `\n--- Creating IAM Service Account for AWS Load Balancer Controller (Attempt ${attempt}/${maxRetries}) ---\n` }));
            await execPromise(command);
            ws.send(JSON.stringify({ type: 'log', data: 'IAM Service Account created successfully.\n' }));
            return true;
        } catch (error) {
            const errorMessage = `Attempt ${attempt} failed: ${error.message}`;
            console.error(errorMessage);
            ws.send(JSON.stringify({ type: 'log', data: `\x1b[33m${errorMessage}\x1b[0m\n` }));

            if (attempt < maxRetries) {
                ws.send(JSON.stringify({ type: 'log', data: `\x1b[33mWaiting ${retryDelay / 1000} seconds before retrying...\x1b[0m\n` }));
                await new Promise(resolve => setTimeout(resolve, retryDelay));
            } else {
                const finalErrorMessage = `Failed to create IAM Service Account after ${maxRetries} attempts.`;
                console.error(finalErrorMessage);
                ws.send(JSON.stringify({ type: 'log', data: `\x1b[31m${finalErrorMessage}\x1b[0m\n` }));
                return false;
            }
        }
    }
...
/**
 * Installs the AWS Load Balancer Controller by applying the embedded Kubernetes manifest.
 * @param {string} clusterName - The name of the cluster.
 * @param {WebSocket} ws - The WebSocket to send logs to.
 * @returns {Promise<boolean>} A promise that resolves to true on success.
 */
async function installAlbControllerManifest(clusterName, ws) {
    // CRITICAL FIX: The full YAML manifest is now embedded directly in the code.
...
`;
    try {
        ws.send(JSON.stringify({ type: 'log', data: `\n--- Applying ALB Controller Kubernetes Manifest from embedded content ---\n` }));
        
        const modifiedManifest = manifestContent.replace(/your-cluster-name/g, clusterName);
        ws.send(JSON.stringify({ type: 'log', data: `Injecting cluster name '${clusterName}' into manifest.\n` }));

        ws.send(JSON.stringify({ type: 'log', data: `Applying the manifest to the cluster...\n` }));
        await execPromise(`kubectl apply -f -`, { input: modifiedManifest });

        ws.send(JSON.stringify({ type: 'log', data: 'AWS Load Balancer Controller manifest applied successfully.\n' }));
        return true;
    } catch (error) {
        const errorMessage = `Failed to apply AWS Load Balancer Controller manifest: ${error.message}`;
        console.error(errorMessage);
        ws.send(JSON.stringify({ type: 'log', data: `\x1b[31m${errorMessage}\x1b[0m\n` }));
        return false;
    }
...
module.exports = {
    applyDefaultStorageClass,
    installCertManager,
    createOrVerifyAlbIamPolicy,
    createAlbIamServiceAccount,
    installAlbControllerManifest
};